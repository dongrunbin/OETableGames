...
import com.oegame.tablegames.model.local.gen.SettingsEntity;
import com.oegame.tablegames.model.local.gen.SettingsDBModel;
...
public class RoomSettingBase implements Serializable
{
	...
	public void init(List<Integer> rule)
	{
		...
		List<SettingsEntity> set = SettingsDBModel.singleton().getList();

		System.out.println("初始化...Begin.");

		for (SettingsEntity entity : set)
		{
			try
			{
		...
		for (int i = 0; i < rule.size(); i++)
		{
			SettingsEntity entity = SettingsDBModel.singleton().get(rule.get(i));
			if (entity != null)
			{
